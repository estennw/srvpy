...
class HJBSolver:
    ...
    def _backtrack_hjb_constL1(self,phi1=None):
        ...
        for k in reversed(range(1,n+m+1)):
            ...
            # Interpolate the optimal direction alpha
            if phi1 == i:
...
class HJBSolverSRS(HJBSolver):
    """
    Class for solving the HJB equation

    max {-Du(x)(a) + <Q1(x1)sqrt(a1), Q2(x2)sqrt(a1)>} = 0

    Where Q1 and Q2 are square root speed (SRS) curves.
    """


    def __init__(self,q1sdt,q2sdt,x1,x2=None,scheme=VInf,unit_sphere=False,normalize=False):
        """
        ...
        # Ensure that the functions are callable with ndarrays as input and
        # find the dimension of the curves
        try:
            q101 = q1(np.array([0,1]))
            self.q1 = q1
        except:
            self.q1 = lambda t: np.array([q1(t) for t in t])
            q101 = self.q1(np.array([0,1]))
        try:
            q201 = q2(np.array([0,1]))
            self.q2 = q2
        except:
            self.q2 = lambda t: np.array([q2(t) for t in t])
            q201 = self.q2(np.array([0,1]))
...
class HJBSolverC(HJBSolverQ):
    ...
    def __init__(self,c1,c2,x1,x2=None,scheme=VInf,unit_sphere=False,normalize=False):
        ...
        try:
            ...
            self.c1 = lambda t: np.array([c1(t) for t in t])
            c101 = self.c1(np.array([0,1]))
        try:
            c201 = c2(np.array([0,1]))
            self.c2 = c2
        except:
            self.c2 = lambda t: np.array([c2(t) for t in t])
            c201 = self.c2(np.array([0,1]))
